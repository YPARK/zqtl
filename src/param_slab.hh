...
void impl_initialize_param(Parameter& P, const tag_param_slab) {
...
auto make_dense_slab(const Index n1, const Index n2, const Opt& opt) {
...
// factory functions
template <typename Scalar, typename Index, typename Opt>
auto make_dense_slab_ptr(const Index n1, const Index n2, const Opt& opt) {
  using Mat = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
  using Param = param_slab_t<Mat, tag_param_dense>;
...
auto make_sparse_slab(const Eigen::SparseMatrixBase<Derived>& A,
                      const Opt& opt) {
...
template <typename Parameter>
void impl_resolve_param(Parameter& P, const tag_param_slab) {
  P.gamma = P.gamma_aux.unaryExpr(P.resolve_prec_op);
  P.theta_var = P.gamma.cwiseInverse();
}

template <typename Parameter>
void impl_resolve_hyperparam(Parameter& P, const tag_param_slab) {
...
template <typename Parameter>
const auto& impl_mean_param(Parameter& P, const tag_param_slab) {
  return P.beta;
}

template <typename Parameter>
const auto& impl_var_param(Parameter& P, const tag_param_slab) {
  return P.theta_var;
}
...
void impl_eval_param_sgd(Parameter& P, const M1& G1, const M2& G2,
                         const M3& Nobs, const tag_param_slab) {
  ////////////////////////////////
  // gradient w.r.t. beta
...
template <typename Parameter>
void impl_check_nan_param(Parameter& P, const tag_param_slab) {

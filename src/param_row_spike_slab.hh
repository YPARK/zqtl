...
struct param_row_spike_slab_t {
  ...
  typedef tag_param_row_spike_slab sgd_tag;
...
void impl_initialize_param(Parameter& P, const tag_param_row_spike_slab) {
...
auto make_dense_row_spike_slab(const Index n1, const Index n2, const Opt& opt) {
  using Mat = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
  using Param = param_row_spike_slab_t<Mat, tag_param_dense>;

  Param ret(n1, n2, opt);
  impl_initialize_param(ret, tag_param_row_spike_slab());
  resolve_param(ret);
...
template <typename Scalar, typename Index, typename Opt>
auto make_dense_row_spike_slab_ptr(const Index n1, const Index n2,
                                   const Opt& opt) {
  using Mat = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
  using Param = param_row_spike_slab_t<Mat, tag_param_dense>;
...
auto make_sparse_row_spike_slab(const Eigen::SparseMatrixBase<Derived>& A,
                                const Opt& opt) {
  const auto n1 = A.rows();
  const auto n2 = A.cols();

  using Mat = Eigen::SparseMatrix<Scalar, Eigen::ColMajor>;
  using Param = param_row_spike_slab_t<Mat, tag_param_sparse>;
...
// mean and variance
template <typename Parameter>
void impl_resolve_param(Parameter& P, const tag_param_row_spike_slab) {
...
template <typename Parameter>
void impl_check_nan_param(Parameter& P, const tag_param_row_spike_slab) {
...
template <typename Parameter>
const auto& impl_mean_param(Parameter& P, const tag_param_row_spike_slab) {
  return P.theta;
}

template <typename Parameter>
auto impl_log_odds_param(Parameter& P, const tag_param_row_spike_slab) {
  return P.alpha_aux;
}

template <typename Parameter>
const auto& impl_var_param(Parameter& P, const tag_param_row_spike_slab) {
  return P.theta_var;
}
...
void impl_eval_param_sgd(Parameter& P, const M1& G1, const M2& G2,
                         const M3& Nobs, const tag_param_row_spike_slab) {
  ////////////////////////////////

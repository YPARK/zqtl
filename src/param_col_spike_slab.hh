...
struct param_col_spike_slab_t {
  ...
  typedef tag_param_col_spike_slab sgd_tag;
  ...
  struct grad_alpha_beta_gamma_tau_t {
    ...
    inline const scalar_t operator()(const scalar_t& b,
      ...
      return half_val * (one_val + std::log(tau_val) - std::log(g) -
...
void impl_initialize_param(Parameter& P, const tag_param_col_spike_slab) {
...
auto make_dense_col_spike_slab(const Index n1, const Index n2, const Opt& opt) {
  using Mat = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
  using Param = param_col_spike_slab_t<Mat, tag_param_dense>;

  Param ret(n1, n2, opt);
  impl_initialize_param(ret, tag_param_col_spike_slab());
  resolve_param(ret);
...
template <typename Scalar, typename Index, typename Opt>
auto make_dense_col_spike_slab_ptr(const Index n1, const Index n2,
                                   const Opt& opt) {
  using Mat = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
  using Param = param_col_spike_slab_t<Mat, tag_param_dense>;
...
auto make_sparse_col_spike_slab(const Eigen::SparseMatrixBase<Derived>& A,
                                const Opt& opt) {
  const auto n1 = A.rows();
  const auto n2 = A.cols();

  using Mat = Eigen::SparseMatrix<Scalar, Eigen::ColMajor>;
  using Param = param_col_spike_slab_t<Mat, tag_param_sparse>;
...
// mean and variance
template <typename Parameter>
void impl_resolve_param(Parameter& P, const tag_param_col_spike_slab) {
...
template <typename Parameter>
void impl_resolve_hyperparam(Parameter& P, const tag_param_col_spike_slab) {
...
template <typename Parameter>
const auto& impl_mean_param(Parameter& P, const tag_param_col_spike_slab) {
  return P.theta;
}

template <typename Parameter>
auto impl_log_odds_param(Parameter& P, const tag_param_col_spike_slab) {
  return P.alpha_aux_col;
}

template <typename Parameter>
const auto& impl_var_param(Parameter& P, const tag_param_col_spike_slab) {
  return P.theta_var;
}
...
void impl_eval_param_sgd(Parameter& P, const M1& G1, const M2& G2,
                         const M3& Nobs, const tag_param_col_spike_slab) {
  ////////////////////////////////
...
void impl_eval_hyperparam_sgd(Parameter& P, const M1& G1, const M2& G2,
  ...
  P.grad_tau_aux +=
      P.alpha.cwiseProduct(P.gamma.binaryExpr(P.beta, P.grad_prior_tau_aux_op))
          .sum();
  P.grad_tau_aux /= ntot;
...
void impl_write_param(Parameter& P, const std::string hdr, const std::string gz,
  ...
  TLOG("Pi = " << std::setw(10) << P.pi_val << " [" << std::setw(10) << P.pi_aux
               << "] Tau = " << std::setw(10) << P.tau_val << " ["
